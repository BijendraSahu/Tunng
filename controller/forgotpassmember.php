...
if($_SERVER['REQUEST_METHOD']=='POST')
{
    ...
    empty($errorlist);
      ...
      if(empty($errorlist))
     {
       ...
       include 'connection.php';
        $sqlc= mysql_query("select * from user_reg where email_id='$email'");
        echo mysql_error();
        if(mysql_num_rows($sqlc)==1)
        {
            ...
            mysql_close();
                ...
                $msg="Thanks to visit <a href='http://www.tunng.co'>tunng.co</a><br/>
                       Your User Id:-<label style='color:red;'>".$result['user_name']."</label>,&nbsp;&nbsp;
                       and Password is:-<label style='color:red;'>".$result['password']."</label>.<br/><br/> ";
                $header=array();
	...
	if($mail->Send())
	{
		   $_SESSION['successmail']="We have sent password in your registered Email id. ";
                       header("location:../forgotpassmember_view.php"); 
	}
	else
	{
