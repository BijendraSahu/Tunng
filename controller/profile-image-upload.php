	...
	if ( isset($_FILES['image']) ) {
		...
		if (! $_FILES['image']['error'] && $_FILES['image']['size'] < $max_file_size) 
			...
			if (in_array($ext, $valid_exts)) {
					...
					$path ='../assets/img/profile/'.$uniqid.'.'.$ext;  
                                        $path2 =$uniqid.'.'.$ext;
                                        //echo $path2;
                                        include('connection.php');
					$sqlc= mysql_query("Update user_registration SET backg_mod_img='$path2', backg_org_img='$img_path2' where user_name='$email'");
					echo mysql_error();
                                        mysql_close();
...
{
    header('location:../profile-image-upload.php'); 
}
?>
